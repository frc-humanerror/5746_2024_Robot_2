...
public class Intake extends SubsystemBase {
  ...
  PIDController intakePID = new PIDController(0.001, 0.001, 0.001);
  ArmFeedforward feedforward = new ArmFeedforward(.3, .3, 1);
  ...
  public void pull(boolean Bpressed, boolean Apressed, boolean Ypressed, boolean Xpressed) {
    /*if (Bpressed && getAnglePosition(spark1) > 170) {
      spark1.set(.1);
    }
    else if (Bpressed && getAnglePosition(spark1) < 170) {
      spark1.set(-.4);
    }
    else if (Apressed && getAnglePosition(spark1) < 110) {
      spark1.set(.3);
    }
    else if (Apressed && getAnglePosition(spark1) > 110) {
      spark1.set(-.05);
    ...
    if (Bpressed) {
      spark1.set(intakePID.calculate(m_intakeEncoder.getPosition(), 180));
      //spark1.set(feedforward.calculate(1800, .1));
    }
    else if (Apressed) {
      spark1.set(intakePID.calculate(m_intakeEncoder.getPosition(), 0));
      //spark1.set(feedforward.calculate(0, .1));
    }
    else {
      spark1.stopMotor();
    }
